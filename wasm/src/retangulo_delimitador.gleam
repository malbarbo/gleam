...
pub fn retangulo_delimitador_examples() {
  ...
  eq(retangulo_delimitador([]), Retangulo(0, 0))
  eq(retangulo_delimitador([p1]), Retangulo(0, 0))
  eq(retangulo_delimitador([p2, p3]), Retangulo(4, 2))
  eq(retangulo_delimitador([p2, p3, p6]), Retangulo(5, 4))
  eq(retangulo_delimitador([p2, p3, p1, p6, p4, p5]), Retangulo(19, 11))
}
...
fn max(lst: List(Int)) -> Int {
  case lst {
    [] -> 0
    [p] -> p
    [primeiro, ..resto] -> imax(primeiro, max(resto))
  }
}
...
fn min(lst: List(Int)) -> Int {
  case lst {
    [] -> 0
    [p] -> p
    [primeiro, ..resto] -> imin(primeiro, min(resto))
  }
}
...
pub fn main() {
  retangulo_delimitador_examples()
}
