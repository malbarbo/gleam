...
pub fn atualiza_quadrado_examples() {
  ...
  // Aberto
  eq(atualiza_quadrado(aberto, Abrir), aberto)
  eq(atualiza_quadrado(aberto, AdicionarBandeira), aberto)
  eq(atualiza_quadrado(aberto, RemoverBandeira), aberto)

  // Fechado
  eq(atualiza_quadrado(fechado, Abrir), aberto)
  eq(atualiza_quadrado(fechado, AdicionarBandeira), bandeira)
  eq(atualiza_quadrado(fechado, RemoverBandeira), fechado)

  // FechadoComBandeira
  eq(atualiza_quadrado(bandeira, Abrir), bandeira)
  eq(atualiza_quadrado(bandeira, AdicionarBandeira), bandeira)
  eq(atualiza_quadrado(bandeira, RemoverBandeira), fechado)
}
